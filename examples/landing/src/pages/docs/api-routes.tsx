  ...
  return (
    <div>
      ...
      <p className="mt-4 text-lg text-muted-foreground">
        Build REST endpoints alongside your frontend pages. API routes live in <span className="text-foreground">src/api/</span> and share the same process as your pages.
      </p>

      <h2 className="mt-12 text-xl font-bold tracking-tight">File-based API routing</h2>
      <p className="mt-3 text-muted-foreground">
        Like pages, API routes use the file system. A file at <span className="text-foreground">src/api/hello.ts</span> becomes <span className="text-foreground">/api/hello</span>.
      </p>
      <CodeBlock label="src/api/hello.ts" code={`import type { ApiHandler } from "@x/core";
...
export const GET: ApiHandler = ({ request }) => {
  return Response.json({ message: "Hello from x!" });
      ...
      <p className="mt-3 text-muted-foreground">
        Each exported HTTP method receives the request and returns a standard <span className="text-foreground">Response</span> object. Dynamic segments work the same as pages: <span className="text-foreground">api/users/[id].ts</span> → <span className="text-foreground">/api/users/:id</span>.
      </p>
      <CodeBlock label="src/api/users.ts" code={`import type { ApiHandler } from "@x/core";
...
export const POST: ApiHandler = async ({ request }) => {
  ...
  return Response.json(result, { status: 201 });
      ...
      <CodeBlock label="src/api/contact.ts" code={`import type { ApiHandler } from "@x/core";
...
export const POST: ApiHandler = async ({ request }) => {
  ...
  return Response.json({ success: true });
      ...
      <CodeBlock label="file tree" lang="tree" code={`src/api/
