  ...
  return (
    <div>
      ...
      <p className="mt-3 text-muted-foreground">
        By default, pages are server-rendered (SSR). Export <span className="text-foreground">mode = "static"</span> to prerender at build time.
      </p>
      ...
      <CodeBlock label="src/pages/about.tsx" code={`import type { RouteProps } from "@x/core";
...
export default function About({}: RouteProps) {
  return (
    <div className="max-w-2xl mx-auto py-12">
      ...
      <p className="mt-3 text-muted-foreground">
        Server pages (the default) run a <span className="text-foreground">loader</span> function on every request. The loader can fetch data, query a database, or call an external API.
      </p>
      <CodeBlock label="src/pages/products.tsx" code={`import type { RouteProps, LoaderArgs } from "@x/core";
...
export default function Products({ loaderData }: RouteProps<typeof loader>) {
  return (
    <div>
      ...
      <p className="mt-3 text-muted-foreground">
        Combined with dynamic routing, loaders receive <span className="text-foreground">params</span> parsed from the URL path.
      </p>
      <CodeBlock label="src/pages/products/[id].tsx" code={`import type { RouteProps, LoaderArgs } from "@x/core";
...
export default function ProductDetail({ loaderData }: RouteProps<typeof loader>) {
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-muted-foreground">
        The <span className="text-foreground">RouteProps</span> type provides typed access to <span className="text-foreground">loaderData</span>, <span className="text-foreground">params</span>, and <span className="text-foreground">request</span>. Pass your loader function as the type parameter for full type safety.
      </p>
      <CodeBlock label="type usage" code={`import type { RouteProps } from "@x/core";
