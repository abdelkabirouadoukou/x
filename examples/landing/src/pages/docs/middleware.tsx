...
export default function DocPage({}: RouteProps) {
  return (
    <div>
      ...
      <p className="mt-3 text-muted-foreground">
        Place a <span className="text-foreground">_middleware.ts</span> file in any route directory. It runs for all routes in that directory and its subdirectories.
      </p>
      <CodeBlock label="file tree" lang="tree" code={`pages/
  ...
  admin/
    ...
    index.tsx`} />
      ...
      <p className="mt-3 text-muted-foreground">
        A middleware function receives an object with <span className="text-foreground">params</span> (dynamic route params), <span className="text-foreground">request</span> (the original Request), and a <span className="text-foreground">next</span> function to continue the chain.
      </p>
      <CodeBlock label="src/pages/_middleware.ts" code={`import type { MiddlewareContext, MiddlewareNext } from "@x/core";
...
export async function middleware(ctx: MiddlewareContext, next: MiddlewareNext) {
  ...
  return next(ctx);
      ...
      <CodeBlock label="src/pages/dashboard/_middleware.ts" code={`import type { MiddlewareContext, MiddlewareNext } from "@x/core";
...
export async function middleware(ctx: MiddlewareContext, next: MiddlewareNext) {
  ...
  return next(ctx);
      ...
      <p className="mt-3 text-muted-foreground">
        Call <span className="text-foreground">next(ctx)</span> to pass control to the next middleware or the route handler. You can modify <span className="text-foreground">ctx.params</span> to enrich the request context for downstream handlers.
      </p>

      <h2 className="mt-12 text-xl font-bold tracking-tight">Redirect patterns</h2>
      <p className="mt-3 text-muted-foreground">
        Return a <span className="text-foreground">Response</span> with a 302 status and a <span className="text-foreground">Location</span> header to redirect. You can also return JSON responses for API middleware validation errors.
      </p>
      <CodeBlock label="redirect example" code={`// Redirect to login
return new Response(null, {
  status: 302,
