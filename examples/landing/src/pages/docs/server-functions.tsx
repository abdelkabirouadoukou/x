...
export default function DocPage({}: RouteProps) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Server Functions</p>
      <h1 className="mt-4 text-3xl font-bold tracking-tight sm:text-4xl">Server functions</h1>
      <p className="mt-4 text-lg text-muted-foreground">
        Call server-side functions directly from the browser without writing REST endpoints. Server functions live in <span className="text-foreground">src/actions/</span> and are invoked via fetch.
      </p>

      <h2 className="mt-12 text-xl font-bold tracking-tight">Defining server functions</h2>
      <p className="mt-3 text-muted-foreground">
        Create a file in <span className="text-foreground">src/actions/</span> and export named async functions. Each function receives a <span className="text-foreground">Request</span> object and any arguments you pass.
      </p>
      <CodeBlock label="src/actions/greet.ts" code={`export async function greet(name: string) {
...
export async function sendEmail({ to, subject, body }: {
  ...
  return { sent: true, to };
      ...
      <p className="mt-3 text-muted-foreground">
        Server functions are called by sending a POST request to <span className="text-foreground">/__x/actions/&lt;filename&gt;/&lt;functionName&gt;</span>. The arguments are sent as JSON in the request body.
      </p>
      <CodeBlock label="client component" code={`"use client";
...
export default function GreetForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <CodeBlock label="src/pages/dashboard.tsx" code={`import type { RouteProps, LoaderArgs } from "@x/core";
