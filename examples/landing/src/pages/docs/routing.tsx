  ...
  return (
    <div>
      ...
      <p className="mt-4 text-lg text-muted-foreground">
        x uses the file system as your route table. Drop a file in <span className="text-foreground">src/pages/</span>, get a route.
      </p>

      <h2 className="mt-12 text-xl font-bold tracking-tight">How it works</h2>
      <p className="mt-3 text-muted-foreground">
        Every <span className="text-foreground">.tsx</span> file in your pages directory becomes a route. The file path determines the URL pattern.
      </p>
...
pages/_404.tsx          -> catch-all 404`} />
      ...
      <p className="mt-3 text-muted-foreground">
        Simple files map to exact URL paths. <span className="text-foreground">pages/about.tsx</span> becomes <span className="text-foreground">/about</span>.
      </p>
  ...
  return <h1 className="text-3xl font-bold">About us</h1>;
      ...
      <p className="mt-3 text-muted-foreground">
        Wrap a filename in square brackets to create a dynamic segment. The value is available via the <span className="text-foreground">params</span> object in loaders.
      </p>
...
export default function BlogPost({ loaderData }: RouteProps<typeof loader>) {
  return (
    <article>
      ...
      <p className="mt-4 text-muted-foreground">
        Multiple dynamic segments work too: <span className="text-foreground">pages/product/[category]/[id].tsx</span> → <span className="text-foreground">/product/:category/:id</span>.
      </p>

      <h2 className="mt-12 text-xl font-bold tracking-tight">Nested routes with folders</h2>
      <p className="mt-3 text-muted-foreground">
        Organize routes in folders for nested URL structures. Each folder can have its own <span className="text-foreground">index.tsx</span>.
      </p>
  ...
  billing/
    ...
    history.tsx       -> /dashboard/billing/history`} />
      ...
      <p className="mt-3 text-muted-foreground">
        Create <span className="text-foreground">pages/_404.tsx</span> to show a custom not-found page for unmatched routes.
      </p>
