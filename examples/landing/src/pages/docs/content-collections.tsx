...
export default function DocPage({}: RouteProps) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Content Collections</p>
      ...
      <p className="mt-3 text-muted-foreground">
        Point the content directory in <span className="text-foreground">x.config.ts</span> to a folder with your markdown files.
      </p>
...
export default defineConfig({
  contentDir: "content",
      ...
      <p className="mt-3 text-muted-foreground">
        Each markdown file starts with frontmatter (YAML between <span className="text-foreground">---</span> delimiters) followed by markdown content.
      </p>
      <CodeBlock label="content/posts/hello-world.md" lang="markdown" code={`---
title: Hello World
date: 2026-03-15
...
\`\`\``} />
      ...
      <p className="mt-3 text-muted-foreground">
        Use <span className="text-foreground">scanContent</span> to discover files and <span className="text-foreground">renderMarkdown</span> to convert markdown to HTML in your loaders.
      </p>
...
export default function BlogPost({ loaderData }: RouteProps<typeof loader>) {
  return (
    <article className="prose max-w-none">
      ...
      <p className="mt-3 text-muted-foreground">
        <span className="text-foreground">scanContent(directory)</span> scans a subdirectory of your content folder and returns an array of content entries. Each entry includes <span className="text-foreground">slug</span>, <span className="text-foreground">frontmatter</span>, and <span className="text-foreground">content</span>.
      </p>

      <h2 className="mt-12 text-xl font-bold tracking-tight">renderMarkdown API</h2>
      <p className="mt-3 text-muted-foreground">
        <span className="text-foreground">renderMarkdown(markdownString)</span> converts markdown to an HTML string. It supports syntax highlighting via Shiki and handles all standard markdown features.
      </p>
